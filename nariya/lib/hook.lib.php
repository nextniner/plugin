...
class NARIYA_STANDARD {
	...
	public function admin_menu($admin_menu){
		...
		$admin_menu['menu010'] = array(
			array('010000', '나리야', NA_ADMIN_URL.'/admin.php', 'nariya'),
		);

		$admin_menu['menu010'][] = array('010100', '나리야 설정', NA_ADMIN_URL.'/admin.php', 'nariya_config');
		if(IS_YC) {
			$admin_menu['menu010'][] = array('010200', '쇼핑몰 메뉴', NA_ADMIN_URL.'/menu_shop.php', 'nariya_menu');
		}
		$admin_menu['menu010'][] = array('010300', '경험치 관리', NA_ADMIN_URL.'/exp.php', 'nariya_exp');
		$admin_menu['menu010'][] = array('010400', '신고글 관리', NA_ADMIN_URL.'/singo.php', 'nariya_singo');
	...
	public function member_login_check($mb, $link, $is_social_login){
		...
		if(IS_EXP) {
			...
			if(!isset($mb['as_per'])) {
				global $g5;
				sql_query(" ALTER TABLE `{$g5['member_table']}` ADD `as_per` tinyint(4) NOT NULL DEFAULT '0' AFTER `mb_10` ");
			}
