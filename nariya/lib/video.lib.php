	...
	switch($info['host']) {
		...
		case 'youtu.be'		   :   
			...
			if($vinfo['type'] == 'shorts') {
				...
				$video['iframe'] = '<div class="plyr_player" data-plyr-provider="youtube" data-plyr-embed-id="'.$video['vid'].'"></div>';
			}
			break;
		...
		case 'vimeo.com' :
			...
			$video['iframe'] = '<div class="plyr_player" data-plyr-provider="vimeo" data-plyr-embed-id="'.$video['vid'].'"></div>';
			break;
...
function na_video($vid, $opt='') {
	...
	$player = '';
	if($iframe) {
		if($video['player'] == 'plyr') {
			$player .= '<div class="na-videowrap rounded">'.PHP_EOL;
			...
			$player .= '<div class="na-videowrap rounded'.$sero_size.'"'.$max_width.'>'.PHP_EOL;
...
function na_video_imgurl($video) {
	...
	if(!$imgurl) {
		...
		curl_setopt($ch, CURLOPT_HEADER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		if($type == "naver" || $type == "tvcast" || $type == "soundcloud") {
		...
		for ($i = 0; $i < $metas->length; $i++) {
			...
			if($meta->getAttribute('property') == "og:image" || $meta->getAttribute('name') == "og:image") {
				if($meta->getAttribute('content')) {
					$img = na_explode('?', $meta->getAttribute('content'));
					$imgurl = isset($img[0]) ? $img[0] : '';
				}
				break;
			}
		}
...
function na_jwplayer($url, $img='', $caption='', $title=''){
	...
	//if($nariya['jw6_video']) {
		if($ext == "mp4" || $ext == "webm") {
			...
			$player = '<div class="na-videowrap rounded"><video class="plyr_player w-100" src="'.$url.'" playsinline controls';
			if($is_auto) {
				$player .= ' autoplay';
			}
			if($img) {
				$player .= ' data-poster="'.$img.'"';
			}
			$player .= '></video></div>';
			...
			$player = '<div><audio class="plyr_player w-100" src="'.$url.'" controls loop';
			if($is_auto) {
				$player .= ' autoplay';
			}
			$player .= '></audio></div>';
