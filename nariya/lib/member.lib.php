...
function na_chk_xp($old_grade, $old_level, $old_exp, $exp) {
	...
	// 레벨업 진행율
	$xp_per = round(($exp - $xp_min) / ($xp_max - $xp_min) * 100);
	...
	return array($grade, $level, $exp, $xp_max, $xp_per, $msg);
}
...
function na_sum_xp($mb) {
	...
	// 레벨변동 체크
	list($grade, $level, $exp, $max, $per, $msg) = na_chk_xp($mb['mb_level'], $mb['as_level'], $mb['as_exp'], $row['sum_exp']);

	// 회원정보 UPDATE
	$msg_sql = ($msg) ? ", as_msg = '{$msg}', as_level = '{$level}' , mb_level = '{$grade}'" : "";

	sql_query(" update {$g5['member_table']} set as_exp = '{$exp}', as_max = '{$max}', as_per = '{$per}' $msg_sql where mb_id = '$mb_id' ");
}
...
function na_insert_xp($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $repeat=0) {
    ...
    $mb = sql_fetch(" select mb_id, mb_level, as_level, as_exp, as_max, as_per from {$g5['member_table']} where mb_id = '$mb_id' ");
    if (!$mb['mb_id']) { return 0; }
...
function na_member($member) {
	...
	$member['as_per'] = isset($member['as_per']) ? $member['as_per'] : 0; // 레벨업 진행율
	...
	// 사용안함
	$member['connect_cnt'] = $member['login_cnt'] = 0;
