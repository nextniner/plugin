...
function na_widget($wname, $wid='', $opt='', $mopt='', $wdir='', $addon=''){
	...
	if($wid) {
		...
		if($is_admin == 'super' || IS_DEMO) {
			$setup_href = NA_URL.'/widget.form.php?lskin='.urlencode(LAYOUT_SKIN).'&amp;wid='.urlencode($wid).'&amp;wname='.urlencode($wname).'&amp;wdir='.urlencode($wdir);
		}
	}
...
function na_sql_find($field, $str, $ex='') {
	...
	*/
	...
	$str = "'".implode("','", na_explode(',', $str))."'";
	$in = ($ex) ? 'NOT IN' : 'IN';
	$sql = " and (".$field." ".$in." (".$str."))";
...
function na_bo_list($gr_list, $gr_except, $bo_list, $bo_except) {
	...
	if($gr_list) {
		...
		// 지정그룹의 게시판 다 뽑기
		$result = sql_query(" select bo_table from {$g5['board_table']} where 1 ".na_sql_find('gr_id', $gr_list));
		if($result) {
	...
	if($wset['wr_notice']) { // 공지글
		...
		if($result) {
			for ($i=0; $row=sql_fetch_array($result); $i++) {
				...
				$result1 = sql_query(" select * from $tmp_write_table where 1 ".na_sql_find('wr_id', $row['bo_notice']));
				if($result1) {
...
function na_tag_post_rows($wset) {
	...
	// 공통쿼리
	$sql_common = " from {$g5['na_tag_log']} where bo_table <> '' ".na_sql_find('tag', $tag)." $sql_plus $sql_minus $sql_mb $sql_term group by bo_table, wr_id ";
