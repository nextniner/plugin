...
// 회원정보 테이블
if(!sql_fetch(" SHOW COLUMNS FROM `{$g5['member_table']}` LIKE 'as_per' ")) {
	sql_query(" ALTER TABLE `{$g5['member_table']}`
					ADD `as_per` tinyint(4) NOT NULL DEFAULT '0' AFTER `mb_10` ", true);
...
if(!sql_fetch(" SHOW COLUMNS FROM `{$g5['member_table']}` LIKE 'as_noti' ")) {
	sql_query(" ALTER TABLE `{$g5['member_table']}`
					ADD `as_noti` int(11) NOT NULL DEFAULT '0' AFTER `as_per` ", true);
