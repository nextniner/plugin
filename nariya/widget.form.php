...
$lskin = isset($_REQUEST['lskin']) ? na_fid($_REQUEST['lskin']) : '';
$is_shop = isset($_REQUEST['shop']) ? na_fid($_REQUEST['shop']) : '';
...
if(isset($_POST['wname']) && $_POST['wname']) {
	...
	// 이동주소
	$goto_url = NA_URL.'/widget.form.php?lskin='.urlencode($lskin).'&amp;wname='.urlencode($wname).'&amp;wid='.urlencode($wid);
...
if($wdir) {
	...
	$widget_path = G5_THEME_PATH.'/layout/'.$lskin.'/widget/'.$wname;
	$widget_url = G5_THEME_URL.'/layout/'.$lskin.'/widget/'.$wname;
}
...
<input type="hidden" name="shop" value="<?php echo $is_shop ?>">
<input type="hidden" name="lskin" value="<?php echo $lskin ?>">
